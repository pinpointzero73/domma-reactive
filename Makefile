...
help:
	...
	@echo "    Actions > Cut a release > patch/minor/major  (notes under ## [Unreleased] first)"
	@echo "    It tests, commits, tags, publishes, then opens the re-pin PR in domma."
	@echo ""
	@echo "  Release by hand  (in this order)"
	...
	@echo "    make release-gh   Push main, tag vX.Y.Z, push the tag - CI then publishes"
	@echo "                      to npm and opens the re-pin PR in domma"
	@echo "    (make release-npm publishes from here instead - only when CI cannot,"
	@echo "     and then the tag's publish run fails and no re-pin PR is opened)"
	@echo ""
	...
	@echo ""
	@echo "  Domma pins this package EXACTLY. Publishing opens the re-pin PR there"
	@echo "  either way; to open one for an earlier version:"
	@echo "    Actions > Re-pin downstream > X.Y.Z"
	@echo ""
